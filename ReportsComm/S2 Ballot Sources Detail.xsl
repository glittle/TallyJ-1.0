    ...
    <html>
      ...
      <body>
        <table class="Report">
          ...
          <tr>
            <td colspan="2" width="70%" class="ReportTitle2">
              <xsl:call-template name="Phrase">
                <xsl:with-param name="Key" select="'VotedVotedInPerson'"/>
              </xsl:call-template>
            </td>
            <td align="right" width="30%" class="ReportTitle2">
          ...
          <tr>
            ...
            <td>
              <table>
                <xsl:apply-templates select="$Voters[@Voted='VotedInPerson']">
          ...
          <tr>
            ...
            <td align="right" class="ReportTitle2">
          ...
          <tr>
            ...
            <td>
              <table>
                <xsl:apply-templates select="$Voters[@Voted='DroppedOff']">
                  <xsl:sort select="@LName" order="ascending"/>
                  <xsl:sort select="@FName" order="ascending"/>
                  <xsl:sort select="@AKAName" order="ascending"/>
                </xsl:apply-templates>
          ...
          <tr>
            ...
            <td>
              <table>
                <xsl:apply-templates select="$Voters[@Voted='Mailed']">
                  <xsl:sort select="@LName" order="ascending"/>
                  <xsl:sort select="@FName" order="ascending"/>
                  <xsl:sort select="@AKAName" order="ascending"/>
                </xsl:apply-templates>
          ...
          <!--<tr>
            ...
            <td align="right" class="ReportTitle2">
              <xsl:value-of select="count(Person[@Voted='No' or @Voted='' or not(@Voted)])"/>
            </td>
          </tr>
          <tr>
            <td/>
            <td>
              <table>
                <xsl:apply-templates select="$Voters[@Voted='No' or @Voted='' or not(@Voted)]">
                  <xsl:sort select="@LName" order="ascending"/>
                  <xsl:sort select="@FName" order="ascending"/>
                  <xsl:sort select="@AKAName" order="ascending"/>
                </xsl:apply-templates>
