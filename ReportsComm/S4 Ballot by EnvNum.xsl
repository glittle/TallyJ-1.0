      ...
      <body>
        <table class="Report">
          ...
          <tr>
            <td colspan="2" class="ReportTitle2">
              <xsl:call-template name="Phrase">
                <xsl:with-param name="Key" select="'BallotsByNum'"/>
              </xsl:call-template>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td colspan="2">
              <table>
                <xsl:apply-templates select="$Voters[@EnvNum or @Voted='DroppedOff' or @Voted='Mailed']">
                  <xsl:sort select="@EnvNum" data-type="number" order="ascending"/>
                  <xsl:sort select="@Voted" order="ascending"/>
                </xsl:apply-templates>
              </table>
            </td>
          </tr>
