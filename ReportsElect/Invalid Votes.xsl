      ...
      <body>
        ...
        <table class="Report">
          <tr class="ReportTitle">
            ...
            <td>
              <xsl:call-template name="Phrase">
                <xsl:with-param name="Key" select="'VOTESTATUS'"/>
              </xsl:call-template>
            </td>
            <td>
              <xsl:call-template name="Phrase">
                <xsl:with-param name="Key" select="'VoteStatusDetail'"/>
              </xsl:call-template>
            </td>
            <td>
              <xsl:call-template name="Phrase">
    ...
    <tr>
      ...
      <td class="ReportSmall">
        ...
        <!--<i>
          <xsl:text>&#160;</xsl:text>
          <xsl:call-template name="Phrase">
            <xsl:with-param name="Key" select="concat('BStatus',ancestor::Ballot/@BallotStatus)"/>
          </xsl:call-template>
        </i>-->
      </td>
      <td class="ReportSmall">
      ...
      <td class="ReportSmall">
        <i>
          <xsl:call-template name="Phrase">
            <xsl:with-param name="Key" select="concat('SpoiledType',current()/../@SpoiledGroup)"/>
          </xsl:call-template>
          <xsl:text> - </xsl:text>
          <xsl:value-of select="current()/../@SpoiledDetail"/>
        </i>
      </td>
      <td class="ReportSmall">
        <span>
